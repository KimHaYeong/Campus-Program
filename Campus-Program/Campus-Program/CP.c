...
void Manager_notify()
{
	...
	while (1)
	{
		...
		case 1:
		{
			...
			{
				move = link;// head°¡ °¡¸£Å°´Â ÁÖ¼Ò°ªÀ» pµµ µ¿ÀÏÇÏ°Ô °¡¸£Å²´Ù
				while (move->next != NULL)
					move = move->next;
				move->next = noti;//pÀÇ ´ÙÀ½ °ªÀÌ NULL°ªÀÌ ¾Æ´Ò ¶§ ´ÙÀ½ ¸Þ¸ð¸®·Î ¿Å°Ü lib°¡ °¡¸£Å°´Â °ªÀ» ³Ö¾îÁØ´Ù
			}
			break;
		}
		case 2:
		{
			...
			scanf("%s", &del_title);
			move = rear = link;
			while ((move != NULL) && (strcmp(del_title, move->title) != 0))
			{
				rear = move;
				move = move->next;
			}
		...
		case 4:
		{
			...
			scanf("%s", &del_title);
			move = rear = link; //¸Ç ¾ÕÀ¸·Î Æ÷ÀÎÅÍ º¯¼ö¸¦ ¿òÁ÷ÀÎ´Ù.
			while ((move != NULL) && (strcmp(del_title, move->title) != 0))
			{
				rear = move;
				move = move->next;
